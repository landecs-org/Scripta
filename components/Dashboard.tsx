...
import { Search, Plus, Calendar, FileText, Trash2, Archive, RefreshCw, Copy, Palette } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const filteredActivities = useMemo(() => {
    return activities.filter(a => {
      ...
      if (isTrashView) return matchSearch && a.deleted;
      if (isArchiveView) return matchSearch && a.archived && !a.deleted;
      
      return matchSearch && !a.archived && !a.deleted;
    });
  }, [activities, searchQuery, isTrashView, isArchiveView]);
  ...
  return (
    <div className="flex flex-col h-full max-w-5xl mx-auto px-4 md:px-8 w-full animate-fade-in" onClick={() => setContextMenu(null)}>
      {/* Header */}
      ...
      {/* Context Menu & Modals remain same ... */}
      {contextMenu && (
          <div 
                  ...
                  <div className="grid grid-cols-4 gap-4">
                      {CARD_COLORS.map(color => (
                          <button 
                            ...
                            className="w-12 h-12 rounded-full border-2 border-black/10 dark:border-white/10 shadow-sm transition-transform active:scale-95"
